...
private:
    ...
    std::vector<std::vector<int>> k_means(const std::vector<Point>& points) {
        ...
        const int LOW = 300, MID = 400, HIGH = 500;
        std::vector<Point> ref_pos = {
            Point(LOW, LOW),  Point(MID, LOW),  Point(HIGH, LOW), Point(LOW, MID),   Point(MID, MID),
            Point(HIGH, MID), Point(LOW, HIGH), Point(MID, HIGH), Point(HIGH, HIGH),
        };
        for (int id : job.orders_id) {
